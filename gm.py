...
def get_pr_metrics(username: str, days: int = 7) -> dict:
    ...
    try:
        ...
        # Track merge times for calculation
        merge_times_hours = []
...
def print_report(metrics_list: list, days: int):
    ...
    for metrics in metrics_list:
        ...
        # Collect individual PR merge times for overall calculation
    ...
    if valid_users > 0:
        ...
        # Calculate overall average merge time
    ...
    try:
        with open(output_file, 'w', newline='') as file:
            fieldnames = ['username', 'total_created', 'total_merged', 'total_open', 
                         'total_abandoned', 'merge_rate', 'abandonment_rate', 'average_merge_time_hours', 'error']
...
def save_detailed_csv(metrics_list: list, output_file: str, days: int):
    ...
    try:
        with open(output_file, 'w', newline='') as file:
            fieldnames = ['username', 'pr_number', 'title', 'state', 'created_at', 'closed_at', 'merge_time_hours', 'repository', 'url']
            writer = csv.DictWriter(file, fieldnames=fieldnames)
